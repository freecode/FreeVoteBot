			...
			if (split.length == 2) {
				...
				try {
					...
					if (poll != null) {
						...
						privmsg.getConversable().sendMessage("Poll #" + poll.getId() + ": " + poll.getQuestion() +
	...
	private void vote(final int answerIndex, final int pollId, final Privmsg privmsg) {
		...
		privmsg.getConversable().getServer().getEventManager().addListener(new NoticeFilter() {
			public boolean accept(Notice notice) {
				if (notice.getSenderNick().equals("ChanServ") && notice.getMessage().equals("Permission denied.")) {
					notice.getServer().getEventManager().removeListener(this);
					return false;
				}
						...
						if (System.currentTimeMillis() < time && !poll.isClosed()) {
							...
							if (vote != null) {
								if (vote.getAnswerIndex() == answerIndex) {
									server.sendNotice(new Notice("You've already voted with this option!", null, privmsg.getSender(), server));
								} else {
									vote.setAnswerIndex(answerIndex);
									voteDAO.updateUsersVote(vote);
									server.sendNotice(new Notice("Vote updated!", null, privmsg.getSender(), server));

								}
							} else {
								voteDAO.addUsersVote(mainNick, pollId, answerIndex);
								server.sendNotice(new Notice("Vote cast!s", null, privmsg.getSender(), server));

							}
						} else {
							server.sendNotice(new Notice("Voting is closed for this poll.", null, privmsg.getSender(), server));
